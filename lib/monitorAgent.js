...
var logger = require('pomelo-logger').getLogger('pomelo-admin-rt', __filename);
...
MonitorAgent.prototype.connect = function(port, host, cb) {
  ...
  this.socket.on('connect', function() {
    console.warn("connect to master = ", host + ':' + port);
  ...
  this.socket.on('error', function(err) {
    if (self.state < ST_CONNECTED) {
      ...
      setTimeout(function(){
        console.warn("connect to master = ", host + ':' + port + ' error!!!');
        self.socket.socket.connect(function(){});
      }, 5000);
    } else {
      self.emit('error', err);
    }
  ...
  this.socket.on('disconnect', function(reason) {
    console.warn("disconnect from master = ", host + ':' + port);
